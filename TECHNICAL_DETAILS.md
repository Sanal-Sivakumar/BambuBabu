...
Orca runs through `xvfb-run`, with an argument array, `shell=False`, captured output, and a ten-minute timeout. Ubuntu also needs `xauth`, `libopengl0`, and `libglu1-mesa`; startup and health validate these dependencies. The systemd unit creates a private `/run/bambubabu` directory and supplies it as `XDG_RUNTIME_DIR`, so the GUI-derived CLI has writable runtime state without weakening the read-only checkout. Output is named `<job-uuid>-<printer>.3mf`, which prevents a fallback slice from being mistaken for the preferred-printer slice. Missing profiles, runtime dependencies, or output fail closed.
