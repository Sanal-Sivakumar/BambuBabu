...
def utcnow() -> datetime:
    """Naive UTC for compatibility with SQLite's existing DateTime columns."""
...
class JobStatus(str, enum.Enum):
    ...
    STARTING = "starting"  # Start command sent; waiting for printer confirmation
    ...
    ATTENTION = "attention"  # Ambiguous physical state; admin inspection required
...
class Job(Base):
    ...
    def __repr__(self):
        return (
            f"<Job id={self.id[:8]} file={self.original_filename} status={self.status}>"
        )
